...
for i in range(6):
	num = start + i
	sample = 'ERR' + str(num)
	seq1 = rawdir + sample + '_1.fastq.gz'
	seq2 = rawdir + sample + '_2.fastq.gz'
	seq = seq1 + ' ' + seq2
	...
	os.system('bwa mem -t 8 -v 1 -M nipponbare7.fa ' + seq + '|'
		  'samtools-v1.3 fixmate -O bam - ' + bwa_out + ' && echo --------- fixmate ok ---------')	
	
	### sort, index, generate a sorted bam file and remove the old one
	os.system('samtools-v1.3 sort -@ 8 -T ERRtmp -O bam -o ' + sorted_out + ' ' + bwa_out
